...
for line in lines:
    if line.startswith("cut"):
        CMDS.append(("CUT", int(line.split()[-1])))
    elif line.startswith("deal with"):
        CMDS.append(("D_W_INC", int(line.split()[-1])))
    elif line.startswith("deal int"):
        CMDS.append(("DINT",))
    else:
        assert False
...
def deal_with_inc(deck, inc):
    L = len(deck)
    newdeck = [-1 for _ in range(L)]
    for i in range(L):
        newdeck[(i * inc) % L] = deck[i]
...
def part1() -> int:
    return shuffle(tuple(range(10007))).index(2019)
...
def part2() -> int:
    assert False, "This is to inefficient"
    deck = tuple(range(119315717514047))
