...
def part2():
    ...
    for p in passports:
        ...
        if not rematch(r"\d{4}", p.get("byr", "")) or not (
            1920 <= int(p.get("byr", "")) <= 2002
        ):
            continue

        # iyr (Issue Year) - four digits; at least 2010 and at most 2020.
        if not rematch(r"\d{4}", p.get("iyr", "")) or not (
            2010 <= int(p.get("iyr", "")) <= 2020
        ):
            continue

        # eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
        if not rematch(r"\d{4}", p.get("eyr", "")) or not (
            2020 <= int(p.get("eyr", "")) <= 2030
        ):
            continue
