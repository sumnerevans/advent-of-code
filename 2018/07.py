...
for line in lines:
    ...
    STEPS.add(x)
    STEPS.add(y)
    STEP_DEPENDENCIES[y].add(x)
    STEP_SUBSEQUENTS[x].add(y)
...
def part2() -> int:
    ...
    T = 0
    remaining = set(ans_part1)
    satisfied = set()
    Q = []
    for i in range(WORKERS):
        heapq.heappush(Q, (0, "WORKER AVAILABLE", i))

    workers_available = 0
    doing = set()
    while Q:
        t, action, id_ = heapq.heappop(Q)
        if action == "JOB DONE":
            doing.remove(id_)
            satisfied.add(id_)
            T = max(T, t)
            ...
            heapq.heappush(Q, (t + job_to_time[id_], "WORKER AVAILABLE", 0))

        if workers_available > 0 and remaining:
            for r in remaining:
