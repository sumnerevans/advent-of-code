...
def part1(lines: List[str]) -> int:
    ...
    min_, max_ = seqminmax(Counter(T).values())
    return max_ - min_
...
def part2(lines: List[str]) -> int:
    ...
    @cache()
    def C(a, b, d) -> Tuple[Tuple[str, int],...]:
        # print(" " * (d), "C", a, b, d)
        if d == 1:
            x = defaultdict(int)
            # x[a]+=1
        ...
        CCC = defaultdict(int)
        counts = C(a, I[a + b], d - 1)
        for k, v in counts:
            CCC[k]+=v
        # print("1" * d, counts)
        counts2 = C(I[a + b], b, d - 1)
        # print("2" * d, counts2)
        for k, v in counts2:
            CCC[k] += v
        # counts[a]+=1
        CCC[I[a + b]] += 1
        # counts[b] += 1
        return tuple(CCC.items())

    # print(C(T[0], T[1], 3))
    # print(Counter("BBBCNCC"))
    print(C("N", "C", 3))
    print(Counter("BBNBNBB"))
    # ohea

    CCCCCC = defaultdict(int)
    for a, b in zip(T, T[1:]):
        c1 = C(a, b, 40)
        print("c1", a, b, c1)
        # CCCCCC[b]+=1
    ...
    print(C.cache_info())
    # print(CCCCCC["B"])
    # assert CCCCCC["B"] == 2192039569602
    min_, max_ = seqminmax(CCCCCC.values())
    return max_ - min_
    # print(math.log10(2_192_039_569_602))
    # for _ in range(40):
    #     print(len(T))
    #     n = ""
    #     for a, b in zip(T, T[1:]):
