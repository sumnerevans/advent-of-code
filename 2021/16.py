...
def part1(lines: List[str]) -> int:
    ...
    B = []
    for c in lines[0]:
        B.extend(list(map(int, pbits(int(c, 16), 4))))

    print(B)

    def parse_packet(bits, LL=math.inf, d=0) -> int:
        print("  " * d, "pp", "".join(map(str, bits)))
        nonlocal ans
        version = None
        type = None
        p = 0
        NP = 0
        while p < len(bits) and NP < LL:
            if all(x == 0 for x in bits[p:]):
                print(">>>", "".join(map(str, bits[p:])))
                break
            print("  " * d, "ww", "p", p, "".join(map(str, bits[p:])))
            if version is None:
                version = bitstrtoint((bits[p : p + 3]))
                print("  " * d, "=========")
                ...
                print("  " * d, "lti", lti)
                if lti == 0:
                    L = bitstrtoint(bits[p + 1 : p + 16])
                    p += 16
                    print("  " * d, "L", L)
                    inc = parse_packet(bits[p : p + L], d=d + 1)
                    p += L
                else:
                    L = bitstrtoint(bits[p + 1 : p + 12])
                    print("  " * d, "L", L)
                    p += 12
                    inc = parse_packet(bits[p:], L, d=d + 1)
                    print("  " * d, "inc", inc)
                    p += inc
...
def part2(lines: List[str]) -> int:
    ...
    def parse_packet(bits, LL=math.inf, d=0) -> Tuple[int, List[int]]:
        print("  " * d, "pp", "".join(map(str, bits)))
        version = None
        type = None
        p = 0
        ...
        while p < len(bits) and NP < LL:
            ...
            if version is None:
                ...
                if lti == 0:
                    ...
                    p += 16
                    print("  " * d, "L", L)
                    inc, v = parse_packet(bits[p : p + L], d=d + 1)
                    p += L
                else:
                    L = bitstrtoint(bits[p + 1 : p + 12])
                    print("  " * d, "L", L)
                    p += 12
                    inc, v = parse_packet(bits[p:], L, d=d + 1)
                    print("  " * d, "inc", inc)
                    p += inc
                ...
                if type == 0:
                    ...
                    SPV.append(1 if v[0] > v[1] else 0)
